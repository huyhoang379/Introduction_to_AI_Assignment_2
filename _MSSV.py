...
def rollout(state):
    ...
    if result is None:  # Nếu không có người thắng, trả về 0 (hòa)
        return 0
    return result
...
def backpropagate(node, result):
    while node:
        node.visits += 1
        # Nếu nút hiện tại là của đối thủ, kết quả cần đảo ngược
...
def select_move(cur_state, remain_time):
    ...
    itermax = 1000  # Adjust iterations based on performance tests
